...
export default function BingoSquare({ square, onClick }: Props) {
  return (
    <div
      onClick={onClick}
      className={`p-4 border-2 rounded-lg cursor-pointer transition-all text-center min-h-[100px] flex items-center justify-center shadow-sm hover:shadow-md
        ${square.isCompleted 
    ...
    >
      <p className="text-sm">{square.text}</p>
    </div>
  );
}
